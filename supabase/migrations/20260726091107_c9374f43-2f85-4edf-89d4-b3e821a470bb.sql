...
BEGIN
  ...
  -- Strip keys with JSON null values from the payload so column DEFAULTs apply on INSERT.
  -- Also drop keys that are not actual columns of the target table.
  ...
  -- Column list for UPDATEs (writable columns excluding system-managed ones)
  ...
  -- Column list for INSERTs: only columns actually present (non-null) in the payload,
  -- so missing NOT-NULL columns fall back to their DEFAULTs.
  ...
  IF v_current_row IS NULL OR v_current_version <= p_base_version THEN
    IF v_current_row IS NULL THEN
      -- INSERT: pass only present columns; DEFAULTs fill the rest.
      IF v_insert_cols IS NULL THEN
        -- Nothing to insert beyond identity; let all defaults apply
        IF v_is_profile_values THEN
      ...
      END IF;
    ELSE
      -- UPDATE: use full writable column list; jsonb_populate_record fills missing keys
      -- from the existing row (via the merged jsonb we build below).
